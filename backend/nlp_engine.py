...
def extract_info(note):
    ...
    # Nouveau : détection du stade
    stade_match = re.search(r'\b(stade\s*[IVX]+)', note, re.IGNORECASE)

    # Correction nom patient
    name_match = re.search(
    r'\b(?:Madame|monsieur)\s+([A-Z][a-zéèêîïëäöüç\-]+)\s+([A-Z][a-zéèêîïëäöüç\-]+)',
    note, re.IGNORECASE
)
    ...
    previous_treatments = re.findall(
        r'(?:ancien traitement|traitement précédent)[:\-]?\s*(.+)', note, re.IGNORECASE
    )
    previous_treatments = [
        t.strip().replace("•", "").strip(":-• ").strip()
        for t in previous_treatments if t.strip()
    ]
    previous_treatments = [
        t for t in previous_treatments
        if not re.search(r'\b(docteur|dr)\b', t, re.IGNORECASE)
    ]
    ...
    if result["weight_kg"] and result["height_cm"] and result["protocol"] in protocol_doses:
        ...
        result["error"] = "Données insuffisantes pour le calcul de la posologie (poids, taille ou protocole manquant)."
